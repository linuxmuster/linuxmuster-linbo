...
$(sfdisk -s "$disk")
.
   [ -n "$cylinders" -a "$cylinders" -gt 0 -a -n "$disksize" -a "$disksize" -gt 0 ] >/dev/null 2>&1 || { echo "Festplatten-Geometrie von $disk laesst sich nicht lesen, cylinders=$cylinders, disksize=$disksize, Abbruch." >&2; return 1; }
  fi
...
torrent_watchdog(){
 ...
 while [ $c -lt $timeout ]; do
  ...
  if [ "$line_old" = "$line" ]; then
   [ $c -eq 0 ] || echo "Torrent-Ueberwachung: Download von $image haengt seit $c Sekunden." >&2 | tee -a /tmp/image.log
   c=$(($c+$int))
  else
   line_old="$line"
   c=0
 ...
 if [ "$RC" = "1" ]; then
  echo "Download von $image wurde wegen Zeitueberschreitung abgebrochen." >&2 | tee -a /tmp/image.log
