...
#    fi
   ...
   ;;
   *) # Yet unknown
   echo "Unbekannte Quelle f�r LINBO-Cache: $1" >&2
   ;;
  esac
...
$(sfdisk -s "$disk")
.
   [ -n "$cylinders" -a "$cylinders" -gt 0 -a -n "$disksize" -a "$disksize" -gt 0 ] >/dev/null 2>&1 || { echo "Festplatten-Geometrie von $disk l�sst sich nicht lesen, cylinders=$cylinders, disksize=$disksize, Abbruch." >&2; return 1; }
  fi
 ...
 case "$1" in
  partition) # full partition dump
   if mountpart "$2" /mnt -w ; then
    echo "Bereite Partition $2 (Gr��e=${size}K) f�r Komprimierung vor..." | tee -a /tmp/image.log
    prepare_fs /mnt "$2" | tee -a /tmp/image.log
    echo "Leeren Platz auff�llen mit 0en..." | tee -a /tmp/image.log
...
#    echo "modprobe cloop file=/cache/$4" | tee -a /tmp/image.log
    if test -s /cache/"$4" && modprobe cloop file=/cache/"$4"; then
     ...
     echo "ist aber f�r die differentielle Sicherung notwendig." | tee -a /tmp/image.log
     RC="$?"
    fi
    rmmod cloop >/dev/null 2>&1
    umount /mnt || umount -l /mnt
...
check_status(){
 ...
 umount /mnt || umount -l /mnt
# [ "$RC" = "0" ] && echo "Enth�lt schon eine Version von $2."
 return "$RC"
}
 ...
 if test -s "$imagefile" && modprobe cloop file=/cache/"$imagefile"; then
  ...
  if [ "$(fstype $targetdev)" = "ntfs" ]; then
   ...
   if [ "$(($s1 - $block))" -gt "$s2" ] 2>/dev/null; then
    echo "FEHLER: Cloop Image $imagefile (${s1}K) ist gr��er als Partition $targetdev (${s2}K)" >&2 | tee -a /tmp/image.log
...
torrent_watchdog(){
 ...
 while [ $c -lt $timeout ]; do
  ...
  if [ "$line_old" = "$line" ]; then
   [ $c -eq 0 ] || echo "Torrent-�berwachung: Download von $image h�ngt seit $c Sekunden." >&2 | tee -a /tmp/image.log
   c=$(($c+$int))
  else
   line_old="$line"
   c=0
 ...
 if [ "$RC" = "1" ]; then
  echo "Download von $image wurde wegen Zeit�berschreitung abgebrochen." >&2 | tee -a /tmp/image.log
...
download_torrent(){
 ...
 echo "Starte Torrent-Dienst f�r $image." | tee -a /tmp/image.log
 if [ -e "$complete" ]; then
 ...
 if [ ! -s "$2" -o ! -s "$2".info ]; then # File not there, download all
  ...
  if [ -s "$2".info ]; then
   ...
   if [ -n "$ts1" -a -n "$ts2" -a "$ts1" -gt "$ts2" ] >/dev/null 2>&1; then
    DOWNLOAD_ALL="true"
    echo "Server enth�lt eine neuere ($ts2) Version von $2 ($ts1)."
   elif  [ -n "$fs1" -a -n "$fs2" -a ! "$fs1" -eq "$fs2" ] >/dev/null 2>&1; then
    DOWNLOAD_ALL="true"
    echo "Dateigr��e von $2 ($fs1) im Cache ($fs2) stimmt nicht."
   fi
   rm -f "$2".info.old
  else
   DOWNLOAD_ALL="true"
    ...
    multicast)
     if [ -s /multicast.list ]; then
      ...
      if [ -n "$MPORT" ]; then
       ...
       echo "Konnte Multicast-Port nicht bestimmen, kein Multicast-Download m�glich." >&2
       RC=1
      fi
     else
      echo "Datei multicast.list nicht gefunden, kein Multicast-Download m�glich." >&2
      RC=1
     fi
...
#    download_all "$1" "$2".info "$2".desc "$2".reg "$2".postsync >/dev/null 2>&1
  ...
  echo "Keine neuere Version vorhanden, �berspringe $2."
 fi
 return "$RC"
}
...
upload(){
 ...
 if remote_cache "$4"; then
  ...
  echo "h�chstwahrscheinlich bereits auf dem Server, daher kein Upload." | tee -a /tmp/linbo.log
  sendlog
  return 1
 fi
...
syncr(){
 ...
 if remote_cache "$2"; then
  echo "Cache $2 ist nicht lokal, �berspringe Aktualisierung der Images."
 else
  mountcache "$2" || return "$?"
  cd /cache
  local i
...
initcache(){
 ...
 for i in *.cloop *.rsync; do
  ...
  if [ "$found" = "0" ]; then
   echo "Entferne nicht mehr ben�tigte Imagedatei $i." | tee -a /tmp/image.log
   rm -f "$i" "$i".*
  fi
 done
...
register(){
 ...
 if echo "$client" | grep -qi '[^a-z0-9-]'; then
  ...
  echo "Rechnernamen d�rfen nur Buchstaben [a-z0-9-] enthalten." >&2
  return 1
 fi
 if echo "$group" | grep -qi '[^a-z0-9_]'; then
  echo "Falscher Gruppenname: '$group'," >&2
  echo "Rechnergruppen d�rfen nur Buchstaben [a-z0-9_] enthalten." >&2
  return 1
 fi
 cd /tmp
