...
# partlabel_startconf partitition
partlabel_startconf(){
  [ -s /start.conf ] || return
  local part="$1"
  local line=""
...
# label_allpartitions
label_allpartitions(){
  echo "Partitionslabel schreiben:" | tee -a /tmp/linbo.log
  if [ ! -e /dev/disk/by-uuid ]; then
    echo "Keine Partitionen gefunden!" | tee -a /tmp/linbo.log
    sendlog
    return 1
  fi
  local dev=""
  ...
  local label=""
  local RC=0
  ls -l /dev/disk/by-uuid/ | grep ^l | awk -F\/ '{ print $3 }' | sort -u | while read line; do
    dev="/dev/$line"
    ...
    if [ -n "$label" ]; then
      ...
      if mk_label "$dev" "$(fstype_startconf "$dev")" "$label" 2>&1 | tee -a /tmp/linbo.log; then
        ...
        echo "$msg FEHLER! Gemountet?" | tee -a /tmp/linbo.log
        RC=1
      fi
    fi
  done
...
# update partition devices in start.conf according to labels
update_devices(){
  local line
  local dev
  local devnew
  local label
  grep -i ^dev /start.conf | sort -u | while read line; do
    dev=""; devnew=""; label=""
    dev="$(echo "$line" | awk -F\= '{ print $2 }' | awk '{ print $1 }')"
    label="$(print_partlabel "$dev")"
    [ -z "$label" ] && continue
    devnew="$(print_realdev "$label")"
    [ -z "$devnew" ] && continue
    [ "$dev" = "$devnew" ] && continue
    echo "Korrigiere $dev --> $devnew"
    sed -i "s|$dev |$devnew |g
            s|$dev\t|$devnew\t|g
            s|$dev$|$devnew|g" /start.conf
  done
}
...
mountpart(){
 ...
 # wait for partition, if not labeled
 if [ -z "$label" ]; then
  for i in 1 2 3 4 5; do
   RC="$?"
   [ "$RC" = "0" ] && break
...
format(){
 ...
 if [ -n "$label" ]; then
  case "$fstype" in
   ...
   ext2|ext3|ext4) cmd="mkfs.$fstype -L $label -F -F $partition" ;;
  ...
  case "$fstype" in
   ...
   ext2|ext3|ext4) cmd="mkfs.$fstype -F -F $partition" ;;
...
update_win(){
 ...
 # don't delete custom scripts on os partition
 ...
 (
  ...
  if [ -n "$targetdir" ]; then
   ...
   if [ -n "$bcd" -a -n "$group" ]; then
    echo "Sichere Windows-BCD für Gruppe $group."
    ...
    echo "Sichere Bootloadersektoren fuer Gruppe $group."
    dd if="$disk" of="$bsmbr" bs=446 count=1 2>> /tmp/linbo.log
    # in case of gpt partition label
    if fdisk -l "$disk" | grep -q "GPT"; then
     echo "Sichere GPT-Label fuer Gruppe $group."
...
mk_cloop(){
 ...
 if [ -z "$size" ]; then
  echo "Kann Größe von $2 nicht bestimmen!" 2>&1 | tee -a /tmp/image.log
  return 1
 fi
 local imgsize=0
 case "$1" in
...
cp_cloop_ntfs(){
 ...
 interruptible ntfsclone -q -f --overwrite "$targetdev" /dev/cloop 2>&1 | tee -a /tmp/image.log
 # in case of cluster referenced twice error give sync a second chance
 if (grep -i error /tmp/image.log | grep -i cluster | grep -qi 'referenced twice' && [ ! -e /tmp/.twice.done ]); then
  touch /tmp/.twice
  RC="0"
 elif grep -qi error /tmp/image.log; then
...
cp_cloop(){
 ...
 if test -s "$imagefile" && modprobe cloop file=/cache/"$imagefile"; then
  ...
  if [ "$(fstype $targetdev)" = "ntfs" ]; then
   ...
   # old cp_cloop stuff
   local s1="$(get_partition_size /dev/cloop0)"
...
restore(){
  ...
  *.[Cc][Ll][Oo]*)
   ...
   if [ "$force" = "force" -a "$fstype" = "ntfs" ]; then
    cp_cloop "$1" "$2" ; RC="$?"
    if [ -e /tmp/.twice -a ! -e /tmp/.twice.done ]; then
     echo "Referenced-Twice-Fehler bei ntfsclone, versuche Datei-Sync..."
     sync_cloop "$1" "$2" ; RC="$?"
     touch /tmp/.twice.done
    fi
   else
    sync_cloop "$1" "$2" ; RC="$?"
   fi
...
restore_winact(){
 ...
 if localmode || [ -z "$mac" ] || [ "$mac" = "OFFLINE" ]; then
  ...
  local cache_batchfile="/cache/$image.winact.cmd"
...
syncl(){
 ...
 if [ -n "$is_win" ]; then
  ...
  if [ -s "$bcd_backup" ]; then
   echo "Restauriere die Windows-BCD aus $(basename "$bcd_backup")."
  ...
  if [ -e "$bsmbr" ]; then
   echo "Restauriere Windows-Bootloader aus $(basename "$bsmbr")" | tee -a /tmp/linbo.log
   case "$bsmbr" in
    *bsmbr.*)
  ...
  if fdisk -l "$disk" | grep -q "GPT"; then
   ...
   if [ -s "$gptlabel" ]; then
    echo "Restauriere GPT-Label aus $(basename "$gptlabel")." | tee -a /tmp/linbo.log
 ...
 # restore partition layout if it was altered above
 #[ -n "$repair_parts" ] && partition | tee -a /tmp/linbo.log
 sendlog
 cd / # ; mountcache "$1" -r
 return "$RC"
}
...
clientname(){
 ...
 if localmode; then
  local cachedev="$(grep -iw ^Cache /start.conf | awk -F\= '{ print $2 }' | awk '{ print $1 }')"
  if [ -b "$cachedev" ]; then
