...
grub-mknetdir --modules="$GRUB_EFI32_ISO_MODULES" -d "$EFI32_DIR" --net-directory="$LINBODIR" --subdir="$SUBDIR"
grub-mknetdir --modules="$GRUB_EFI64_ISO_MODULES" -d "$EFI64_DIR" --net-directory="$LINBODIR" --subdir="$SUBDIR"
mv "$CORE_EFI32.efi" "$CORE_EFI32.iso"
mv "$CORE_EFI64.efi" "$CORE_EFI64.iso"

# make special purpose minimal netboot image for i386
grub-mknetdir --fonts="$FONTS" -d "$I386_DIR" --net-directory="$LINBODIR" --subdir="$SUBDIR"
mv "$CORE_I386.0" "$CORE_I386.min"

# make standard netboot images
grub-mknetdir --fonts="$FONTS" --modules="$GRUB_I386_MODULES" -d "$I386_DIR" --net-directory="$LINBODIR" --subdir="$SUBDIR"
grub-mknetdir --fonts="$FONTS" --modules="$GRUB_EFI32_MODULES" -d "$EFI32_DIR" --net-directory="$LINBODIR" --subdir="$SUBDIR"
grub-mknetdir --fonts="$FONTS" --modules="$GRUB_EFI64_MODULES" -d "$EFI64_DIR" --net-directory="$LINBODIR" --subdir="$SUBDIR"
